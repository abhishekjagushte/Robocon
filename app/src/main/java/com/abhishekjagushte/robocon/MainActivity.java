...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        Toast.makeText(this, "Height = "+height+" Width = "+width, Toast.LENGTH_SHORT).show();
    }
    ...
    @Override
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        mat1 = inputFrame.rgba(); //Height = 1080 & width = 1440
        //Core.transpose(mat1,mat2);
        //Log.i("Size","Height = "+mat2.height()+" Width = "+mat2.width());

        Imgproc.cvtColor(mat1,mat1,Imgproc.COLOR_RGBA2RGB);
        Mat imageBlob = Dnn.blobFromImage(mat1, 0.00392,new Size(416,416), new Scalar(0,0,0),false, false);

        if(yoloStart) {
            tinyYolo.setInput(imageBlob);
            mat1 = tinyYolo.forward();
        }
        return mat1;
    }
    ...
    public void yolo(View view) {
        String yolo3_cfg = Environment.getExternalStorageDirectory() + "/dnn/yolo3-tiny.cfg";
        String yolo3_weights = Environment.getExternalStorageDirectory() + "/dnn/yolo3-tiny.weights";

        tinyYolo = Dnn.readNetFromDarknet(yolo3_cfg,yolo3_weights);
